...
interv_1.5km <- readRDS("data/interv_gp/1.5km_interv.rds")


gp <- gp %>%  
  mutate(
    interv = case_when(gpcode %in% interv_1.5km  ~ 1, 
                       TRUE ~ 0)
  )
...
c_n_gpcodes <- setdiff(c_n_gpcodes, interv_1.5km)
...
count_1.5km_exgp <- run_microsynth(gp %>% 
...
count_1.5km_exgp_other <- run_microsynth(gp %>% 
                                     filter(time >= 13 & gpcode %notin% c_n_gpcodes), 
                                   idvar = "gpcode", timevar = "time", 
                                   intvar = "interv", start.pre = 13, end.pre = 19, 
                                   end.post = 24, match.out = match.out, match.covar = cov.var,
...
### table -----
count_1.5km_exgp_tbl <- extract_syn_results(count_1.5km_exgp, count_1.5km_exgp_other) %>% 
  gt() %>% 
  tab_header(
  ...
  tab_footnote(
    footnote = glue(
      "Model parameters: start.pre = {count_1.5km_exgp$info$start.pre}, ",
      "end.pre = {count_1.5km_exgp$info$end.pre}, ",
      "end.post = {count_1.5km_exgp$info$end.post}"),
...
p_count_1.5km_exgp  <- extract_svy_ratios(
  count_1.5km_exgp, outcome_denom_map = out_den,  synth.df = gp %>% 
    filter(time >= 13 & gpcode %notin% c_n_gpcodes))

count_1.5km_exgp_plot <- plot_svy_ratios(p_count_1.5km_exgp, ncol = 1, endpre = count_1.5km_exgp$info$end.pre)
...
p_count_1.5km_exgp_other  <- extract_svy_ratios(
  count_1.5km_exgp_other, outcome_denom_map = out_den,  synth.df = gp %>% 
    filter(time >= 13 & gpcode %notin% c_n_gpcodes))

count_1.5km_exgp_plot_other <- plot_svy_ratios(p_count_1.5km_exgp_other, ncol = 1, 
                                             endpre = count_1.5km_exgp_other$info$end.pre)
